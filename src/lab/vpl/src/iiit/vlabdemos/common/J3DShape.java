...
//       Material(Color3f ambientColor, Color3f emissiveColor, Color3f diffuseColor, Color3f specularColor, float shininess) 
    ...
    public Group createConewithTransparency(Vector3d pos,Vector3d scale,Vector3d rot,Color3f colr) {
        // Create a transform group node to scale and position the object.
    	//new Point3d(0.0, 0.0, 0.0)
 	   Transform3D t = new Transform3D();
        float rad = (float)Math.PI/180;
        ...
        app.setColoringAttributes(ca);
        app.setTransparencyAttributes(new TransparencyAttributes(TransparencyAttributes.NICEST,.3f));
...
//         polygon1.setTextureCoordinate (3, new Point2f(0.0f,1.0f));
    ...
    public Group createRectangle1(Vector3d pos,Vector3d scale,Color3f colr,String texfile) { //,
        // Create a transform group node to scale and position the object.
...
//        polygon1.setTextureCoordinate (3, new Point2f(0.0f,1.0f));
	   ...
	   app.setRenderingAttributes(cool);
	    ...
	    Shape3D shape = new Shape3D(plane,app);
	    shape.setCapability(Shape3D.ALLOW_GEOMETRY_WRITE | Shape3D.ALLOW_GEOMETRY_READ | Shape3D.ENABLE_PICK_REPORTING);
...
//    	Anti-clockwise
    ...
    public Geometry createlinegeometry(Point3d p1, Point3d p2,Color3f colr)
    {
    	 int size=2;
...
// 	    
  	...
  	public Group createTriangle(Point3d point1,Point3d point2,Point3d point3,Vector3d scale,Vector3d rot,Color3f color,String texfile)
    {
  		Transform3D t = new Transform3D();
  	    float rad = (float)Math.PI/180;
  			if(rot.x != 0)
        ...
        TriangleArray tri = new TriangleArray(3, TriangleArray.COORDINATES|TriangleArray.COLOR_3 | GeometryArray.TEXTURE_COORDINATE_2);
        tri.setCoordinate(0, point1);
  	    tri.setCoordinate(1, point2);
  	    tri.setCoordinate(2,point3) ;
  	    tri.setColor(0, color);
        ...
        q.set(0.0f, 1.0f);
 	    tri.setTextureCoordinate(0, 0, q);
 	    q.set(0.0f, 0.0f);
 	    tri.setTextureCoordinate(0, 1, q);
 	    q.set(1.0f, 0.0f);
 	    tri.setTextureCoordinate(0, 2, q);
  	...
  	public Group createRectangle(Point3d p1, Point3d p2, Point3d p3, Point3d p4,Vector3d pos,Vector3d scale,Color3f colr,String texfile) 
  	{ 
  		//,
        // Create a transform group node to scale and position the object.
	    ...
	    TexCoord2f q = new TexCoord2f();
	    q.set(0.0f, 1.0f);
	    polygon1.setTextureCoordinate(0, 0, q);
	    q.set(0.0f, 0.0f);
	    polygon1.setTextureCoordinate(0, 1, q);
	    q.set(1.0f, 0.0f);
	    polygon1.setTextureCoordinate(0, 2, q);
	    q.set(1.0f, 1.0f);
	    polygon1.setTextureCoordinate(0, 3, q);
	    ...
	    Shape3D shape = new Shape3D(polygon1,app);
	    shape.setCapability(Shape3D.ALLOW_GEOMETRY_WRITE | Shape3D.ALLOW_GEOMETRY_READ);
     ...
     /*   Dimension dim=new Dimension(1,1);
        ...
        objtrans.addChild(new Rectangle(new Point3d(-0.3,0.5,-0.2),dim)); */
        
        return objtrans;
   }

  	
  	public Appearance GetAppearance(String imagePath)
  	{
  		Appearance app= new Appearance();
    	Texture tex = new TextureLoader(Resources.getResource(imagePath),TextureLoader.BY_REFERENCE | TextureLoader.Y_UP,this).getTexture();
    	app.setTexture(tex);
	    TextureAttributes texAttr = new TextureAttributes();
	    texAttr.setTextureMode(TextureAttributes.MODULATE);
	    app.setTextureAttributes(texAttr);
  	...
  	public Group createTexturedSphere( Vector3d pos,float radius,Color3f ambient, Color3f emissive,Color3f diffuse,Color3f specular,float shine ,String TextureImagePath)
  	{
  		 Transform3D tr2 = new Transform3D();
  	   ...
  	   Appearance app = new Appearance();
      	...
      	Texture tex = new TextureLoader(Resources.getResource(TextureImagePath),TextureLoader.BY_REFERENCE | TextureLoader.Y_UP,this).getTexture();
        tex.setBoundaryModeS(Texture.WRAP);
        tex.setBoundaryModeT(Texture.WRAP);
       // tex.setBoundaryColor( new Color4f( 0.0f, 1.0f, 0.0f, 0.0f ) );
  		app.setTexture(tex);
  	 ...
  	 int primflags = Primitive.GENERATE_NORMALS + Primitive.GENERATE_TEXTURE_COORDS; 
  	   Sphere mySphere = new Sphere(radius ,primflags, 120,app );
  	   sphereGroup.addChild(mySphere);
  	   
  	   return sphereGroup;
  	
  		
  	}
   public Group createMyRectangle(Vector3d pos,Vector3d scale,Color3f colr,String texfile) { //,
        // Create a transform group node to scale and position the object.
...
//        polygon1.setTextureCoordinate (3, new Point2f(0.0f,1.0f));
   ...
   public Group CreateCircle(Vector3d pos,Vector3d scale,Vector3d rot,double angle,float radius,Color3f colr,Color3f ambientColor, Color3f emissiveColor, Color3f diffuseColor, Color3f specularColor, float shininess)
   {
	  Transform3D t = new Transform3D();
	  float rad = (float)Math.PI/180;
	   if(rot.x != 0)
    ...
    app.setColoringAttributes(ca);
	  ...
	  int size=0;
	  double previousy=0;
	  double previousx=1*radius;
	 
	  for(x=0;x<=angle; x=x+(1.0f/40.0f))
	  {
		  size++;
		 
	  }
	 
	  LineArray line =new LineArray((size-2)*2,LineArray.COORDINATES | LineArray.COLOR_3);
	  
	  Point3d pnt_verts[] = new Point3d[(size-2)*2];
	  int i=0;
	  int count=0;
	  for(x=0;i<size&&count<(size-2)*2;x+=(1.0f/40.0f),i++)
	  {
		    double x_cord = radius*Math.cos(rad*x);
	 	 	double y= radius*Math.sin(rad*x);
	 	 	
	 	 	line.setColor(count,colr);
	 	 	pnt_verts[count++]=new Point3d(previousx,previousy,0);
	 	 	line.setColor(count,colr);
	 	 	pnt_verts[count++]=new Point3d(x_cord,y,0);
	 	 	previousx=x_cord;
	 	 	previousy=y;
