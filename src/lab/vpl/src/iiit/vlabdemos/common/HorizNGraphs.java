...
public class HorizNGraphs extends JPanel implements MouseListener, MouseMotionListener 
{
	...
	public void createGraph(Graphics g)
	{
	  ...
	  g.drawLine(m_x_offset, m_y_offset, m_width - m_x_offset, m_y_offset);
	   g.drawLine(m_x_offset, m_y_offset - (int)(m_height*0.7), m_x_offset, m_y_offset + (int)(m_height*0.7));
	...
	private void drawAllText(Graphics g){
		...
		if(m_Vdisplay){
			...
			for(; i<m_y_str.length();y+=10,i++)
				g.drawString(m_y_str.substring(i,i+1), 10, m_y_offset - (int)(m_height*0.7) + y);
		...
		for(i=0;i<m_numOfGraphs;i++){
			...
			g.drawString( m_nCaptions[i] + " : ___ " , (int)(m_width*.7),40+ 10*i);
	...
	private void showScale(Graphics g)
	{
	    ...
	    for(int x= m_x_offset + y_step ; x<m_width - m_x_offset;x+=y_step)
	    	g2d.drawLine(x,m_y_offset - (int)(m_height*0.7), x,m_y_offset + (int)(m_height*0.4));
	    
	    Font cur_Font = new Font("Arial", Font.BOLD, 8);
		g.setFont(cur_Font);
		
	    int i=y_step;
	    for(int y= m_y_offset-y_step ; y>m_y_offset - (int)(m_height*0.7) ;y-=y_step, i+=y_step)
	    	g.drawString(i*(1.0/m_scale_y) + "",m_width - m_x_offset,y);
	    i=-y_step;
	    for(int y= m_y_offset + y_step; y<m_y_offset + (int)(m_height*0.7);y+=y_step, i-=y_step)
	    	g.drawString(i*(1.0/m_scale_y) + "",m_width - m_x_offset,y);
...
public void addGraphValue (float val) {
