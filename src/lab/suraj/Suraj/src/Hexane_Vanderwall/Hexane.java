 ...
 */
public class Hexane extends javax.swing.JPanel  implements MouseListener, MouseMotionListener,MouseWheelListener{
...
// 	 	assign the visibility mask to the Switch
     ...
     */
    public Hexane(Container container) {
...
//        scene.addChild(bgleg);
    ...
    private static class MyFrame extends JFrame {
        MyFrame() {
            ...
            getContentPane().add(new Hexane(this), BorderLayout.CENTER);
            pack();
        }
    }
    ...
    public static void nonbond_force () throws NumberFormatException, IOException
	{
    	...
    	for (int j=2;j<= iter_nonbond ;j++)
		{
			...
			fx_nonbond_i[nonbond1[j]]+= (du_dr)* du_dxi ;
			fy_nonbond_i[nonbond1[j]]+= (du_dr)* du_dyi ;
			fz_nonbond_i[nonbond1[j]]+= (du_dr)* du_dzi ;
			
			fx_nonbond_j[nonbond1[j]]-= (du_dr)* du_dxi ;
			fy_nonbond_j[nonbond1[j]]-= (du_dr)* du_dyi ;
			fz_nonbond_j[nonbond1[j]]-= (du_dr)* du_dzi ;
		...
		if (fnet > -2.1088174E-7 )
			...
			{
				counterrr++;
				ftotal_nonbond=(float) ((fnet/counterrr)*(magnet_y_translate1+magnet_x_translate+magnet_z_translate+magnet_x1_translate+magnet_z1_translate+magnet_y1_translate1));
			}
		}
...
/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
    ...
    public static void nonbond_energy2 () throws NumberFormatException, IOException
	{
		...
		for (int j=2;j<= row ;j++)
		{
			...
			float pow12=(float) Math.pow(rho_r,12);
			float pow6=(float) Math.pow(rho_r,6);
			float uvanderval=(float) (4*tempe*(pow12-pow6));
	...
	//		System.out.println("rho_r "+rho_r);
			if (uvanderval > -2.1088174E-7 )
				utotal_nonbonded=utotal_nonbonded+(uvanderval*1000000);//+electrostatic;
		 }
		ftotal_nonbond=(float) (2*utotal_nonbonded-2);
		}
		    ...
		    public void stateChanged(ChangeEvent e) {
				...
				try {
				 ...
				 try {
					...
					Float distance_single=(float) (Math.sqrt(Math.pow((magnet_y_translate1-magnet_y1_translate1),2))+Math.pow((temp-magnet_x1_translate-1),2)+Math.pow((magnet_z_translate-magnet_z1_translate),2));
			    	...
			    	{   magnet_x_translate=temp;
				    	...
				    	objtr.getTransform(tr);
				    	tr.setTranslation(new Vector3d(temp-0.5,magnet_y_translate1+displacement+0.25,magnet_z_translate));
				    	objtr.setTransform(tr);		
	   	...
	   	slider_y = new JSlider(JSlider.HORIZONTAL, -50, 50,0);
		    slider_y.addChangeListener(new ChangeListener() {
				...
				////System.out.println("disp"+disp);
			  		...
			  		//disp=0;*/
				Float distance_single=(float) (Math.sqrt(Math.pow((temp-magnet_y1_translate1),2))+Math.pow((magnet_x_translate-magnet_x1_translate-1),2)+Math.pow((magnet_z_translate-magnet_z1_translate),2));
		    	System.out.println("Distance "+distance_single);
		    	if (distance_single > 0.5)
		    	{
			    	...
			    	objtr.getTransform(tr);
			    	tr.setTranslation(new Vector3d(magnet_x_translate-0.5,temp+displacement+0.25,magnet_z_translate));
			    	objtr.setTransform(tr);	
		    ...
		    slider_z.addChangeListener(new ChangeListener() {
				...
				try {
					...
					e1.printStackTrace();
			  		...
			  		//disp=0;*/
				Float distance_single=(float) (Math.sqrt(Math.pow((magnet_y_translate1-magnet_y1_translate1),2))+Math.pow((magnet_x_translate-magnet_x1_translate-1),2)+Math.pow((temp-magnet_z1_translate),2));
		    	System.out.println("Distance "+distance_single);
		    	if (distance_single > 0.5)
		    	{
		    	...
		    	objtr.getTransform(tr);
		    	tr.setTranslation(new Vector3d(magnet_x_translate-0.5,magnet_y_translate1+displacement+0.25,temp));
		    	objtr.setTransform(tr);	
     ...
     lbl = new JLabel("X coordinate", JLabel.LEFT);
	    ...
	    slider_x.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
				...
				Float distance_single=(float) (Math.sqrt(Math.pow((magnet_y_translate1-magnet_y1_translate1),2))+Math.pow((magnet_x_translate-temp-1),2)+Math.pow((magnet_z_translate-magnet_z1_translate),2));
		    	System.out.println("Distance "+distance_single);
		    	if (distance_single > 0.5)
		    	{
		    	...
		    	objtr.getTransform(tr);
		    	tr.setTranslation(new Vector3d(temp+0.5,magnet_y1_translate1+displacement+0.25,magnet_z1_translate));
		    	objtr.setTransform(tr);
	    ...
	    slider_x.setBackground(bk);
	   	...
	   	slider_y = new JSlider(JSlider.HORIZONTAL, -50*2, 50*2,0);
		    slider_y.addChangeListener(new ChangeListener() {
				...
				try {
				 ...
				 try {
					...
					e1.printStackTrace();
			  		...
			  		//disp=0;*/
				Float distance_single=(float) (Math.sqrt(Math.pow((temp-magnet_y_translate1),2))+Math.pow((magnet_x_translate-magnet_x1_translate-1),2)+Math.pow((magnet_z_translate-magnet_z1_translate),2));
		    	System.out.println("Distance "+distance_single);
		    	if (distance_single > 0.5)
		    	{	
		    	...
		    	objtr.getTransform(tr);
		    	tr.setTranslation(new Vector3d(magnet_x1_translate+0.5,temp+displacement+0.25,magnet_z1_translate));
		    	objtr.setTransform(tr);	
	 ...
	 lbl = new JLabel("Z coordinate ", JLabel.LEFT);
	   	slider_z = new JSlider(JSlider.HORIZONTAL, -50*2, 50*2,0);
		    slider_z.addChangeListener(new ChangeListener() {
				...
				Float distance_single=(float) (Math.sqrt(Math.pow((magnet_y_translate1-magnet_y1_translate1),2))+Math.pow((magnet_x_translate-magnet_x1_translate-1),2)+Math.pow((temp-magnet_z_translate),2));
		    	System.out.println("Distance "+distance_single);
		    	if (distance_single > 0.5)
		    	{
		    	...
		    	tr.setTranslation(new Vector3d(magnet_x1_translate+0.5,magnet_y1_translate1+displacement+0.25,temp));
		    	objtr.setTransform(tr);	
...
//	 enableStage(-2);
 ...
 /*           for(int k=0;k<poly_count;k++)
  ...
  //          	tr.setTranslation(new Vector3d(poly_centers[k][0],poly_centers[k][1],poly_centers[k][2]));
        	...
        	tr.setTranslation(new Vector3d(-0.5,displacement+0.25,0));
        	objtr.setTransform(tr);
