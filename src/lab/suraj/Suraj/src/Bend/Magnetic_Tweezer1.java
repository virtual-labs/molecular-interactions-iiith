...
/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
	...
	/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
		...
		public static void force_bend(int cas)
		{
			ftotal_bend=0;
			counterr++;
		...
		/*	if(j==2)
			...
			fz_j[bend2[j]]=-sint*dq_dxk*dv_dt+fx_k[bend3[j]];
			fy_k[bend3[j]]=-sint*dq_dyk*dv_dt+fy_k[bend3[j]];
			fz_k[bend3[j]]=-sint*dq_dzk*dv_dt+fz_k[bend3[j]];
			
			//ftotal_bend+=(fx_i[bend1[j]]+fy_i[bend1[j]]+fz_i[bend1[j]]+fx_j[bend2[j]]+fz_j[bend2[j]]+fz_j[bend2[j]]+fy_k[bend3[j]]+fz_k[bend3[j]]);
			...
			float fnet=fi+fj+fk;//check -> atom 1 has 3 bends.all calculated separately;
				...
				{
					counterrr++;
					ftotal_bend-=((fnet/counterrr)*magnet_y_translate);
				}
					fnet=0;
			//ftotal_bend=0;
			}
		    ...
		    slider_y.addChangeListener(new ChangeListener() {
			    public void stateChanged(ChangeEvent e) {
			    	...
			    	objtr.setTransform(tr);
			  	   ...
			  	   try {
			  	    utotal_bend =0;
			  	    //if(ftotal_bend>500000 || ftotal_bend<-500000)	
			  	    	//ftotal_bend =0;
				...
				move();
					Energy.setState(1);  	
			    	Energy.setCurrentValue((float)disp/100,(float)((utotal_bend)));      // Note that disp denotes the net displacement.  
			    	Energy.addGraphValue((float)(2*utotal_bend));//-freeBody.getDisplacment()*500))
			    	Energy.drawGraph();
			    	Energy.setYScale(10);
			    	
			    	Force.setState(1);
			    	System.out.println("ftotal  "+ftotal_bend+"position  "+temp);
			    	Force.setCurrentValue((float)disp/200,(float)ftotal_bend);      
			    	float cd=(ftotal_bend);
			        Force.addGraphValue(cd);//-freeBody.getDisplacment()*500));
			        Force.drawGraph();
			        Force.setYScale(10);
