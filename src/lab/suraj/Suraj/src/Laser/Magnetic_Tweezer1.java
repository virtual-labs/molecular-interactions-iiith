package Laser;
//package iiit.vlabdemo.simplependulam;
 ...
 */
public class Magnetic_Tweezer1 extends javax.swing.JPanel  implements MouseListener, MouseMotionListener,MouseWheelListener{
...
//	JButton Start_experiment;
	...
	JSlider slider_sg1;
	JSlider slider_sg2;
	JSlider slider_ep;///////////////////////////////////PPPPPPPPPPPPPPPPPPPPPPPPPPPP
...
//	private FullViewGraph		fullViewGraph= new FullViewGraph();
	...
	private static float magnet_z_translate=(float) 0.0;
	
	private static double magnet_x1_translate=0.0;
	private static float magnet_y1_translate=(float) 0.0;
	private static float magnet_z1_translate=(float) 0.0;
...
//	private double bond_length=1;
	...
	public BranchGroup createSceneGraph() 
	{
		...
		objRoot.addChild(m_j3d.createBox(new Vector3d(0,-0.0, -1.0),new Vector3d(1.5,0.7,0.7),new Vector3d(0,0,0),new Color3f(0f, 1f, 0f),"Laser.jpg"));
...
//		tr=new Transform3D();
    ...
    //	First Atom 	
    	...
    	atom1.addChild(m_j3d.createSphere(new Vector3d(0.1,0,0), 0.06f, new Color3f(1,0,0), new Color3f(1,0,0), new Color3f(1,0,0), new Color3f(1,0,1), 3.0f));
    ...
    //  Second Atom
    	...
    	atom2.addChild(m_j3d.createSphere(new Vector3d(0,0,0), 0.06f, new Color3f(1,0,0), new Color3f(0,1,0), new Color3f(1,0,0), new Color3f(1,0,1), 3.0f));
...
//    	support2.addChild(m_j3d.createCylinder(new Vector3d(0,-0.3,0), new Vector3d(0.4,0.4,0.4), new Vector3d(0,0,90), new Color3f(0.2f,0.5f,0.5f)));
	...
	*/	 	
    ...
    //	plane1.addChild(m_j3d.createBox(new Vector3d(0,-0.2,-0.4),new Vector3d(0.3,0.001,1),new Vector3d(0,0,0),new Color3f(0f, 1f, 0f),"grey4.jpg"));
		//plane.addChild(m_j3d.createBox(new Vector3d(0,-0.34, -.4),new Vector3d(1.323,.03,.52),new Vector3d(0,0,0),new Color3f(0f, 1f, 0f),"table.jpg"));
		//plane1.addChild(m_j3d.createBox(new Vector3d(0,-0.2, -.4),new Vector3d(1.323,.03,.52),new Vector3d(0,0,0),new Color3f(0f, 1f, 0f),"table.jpg"));
		//background.addChild(m_j3d.createBox(new Vector3d(0,0,-2),new Vector3d(2,2,-0.00),new Vector3d(0,0,0),new Color3f(0f, 1f, 0f),"bg.jpg"));
...
// 	 	assign the visibility mask to the Switch
     ...
     */
    public Magnetic_Tweezer1(Container container) {
...
//        scene.addChild(bgleg);
    ...
    public static class MyApplet extends JApplet {
        Magnetic_Tweezer1 mainPanel;

        public void init() {
            setLayout(new BorderLayout());
            mainPanel = new Magnetic_Tweezer1(this);
    ...
    private static class MyFrame extends JFrame {
        MyFrame() {
            ...
            getContentPane().add(new Magnetic_Tweezer1(this), BorderLayout.CENTER);
            pack();
        }
    }
...
/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
	...
	/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
		...
		public static void force_bend()
		{
		 ...
		 public static void energy_dihed () throws NumberFormatException, IOException
			{
		...
		/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
				...
				for (int j=2;j<= iter_dihed ;j++)
				{
					...
					float rz1=(position[dihed2[j]][2]-position[dihed3[j]][2]);
					
					float rx2=(position[dihed3[j]][0]-position[dihed4[j]][0]);
					float ry2=(position[dihed3[j]][1]-position[dihed4[j]][1]);
					float rz2=(position[dihed3[j]][2]-position[dihed4[j]][2]);
		 ...
		 public static void move_single()
			{
			 //float templol=(float) (magnet_x1_translate-magnet_x1_translate);
			 //System.out.println("Look "+templol);
			 distance_single=(float) (Math.sqrt(Math.pow((magnet_y1_translate),2))+Math.pow((magnet_x1_translate),2)+Math.pow((magnet_z1_translate),2));
			 //System.out.println("Look @ distance"+distance_single);
			}
    ...
    /*	for (int j=2;j<= iter_nonbond ;j++)
		{
    		float rx=(position[nonbond1[j]][0]-position[nonbond2[j]][0]);
			float ry=(position[nonbond1[j]][1]-position[nonbond2[j]][1]);
			float rz=(position[nonbond1[j]][2]-position[nonbond2[j]][2]);
			...
			fx_nonbond_i[nonbond1[j]]+=  du_dxi ;
			fy_nonbond_i[nonbond1[j]]+= du_dyi ;
			fz_nonbond_i[nonbond1[j]]+=  du_dzi ;
			
			fx_nonbond_j[nonbond1[j]]-=  du_dxi ;
			fy_nonbond_j[nonbond1[j]]-=  du_dyi ;
			fz_nonbond_j[nonbond1[j]]-=  du_dzi ;
		...
		*/
		//float du_dr=tempe*((12*pow12/distance)-2*(6*pow6/distance));
		//float dq_dr=(float) (-(332.0716*tempq1*tempq2)/(distance*distance));
		ftotal_nonbond=(float) (Konst*distance_single);
...
/*			if (iter>1) //assuming only 1 line header
		...
		/*	float temprho=(temprho1+temprho2)/2;
			...
			*/
			...
			float energy=(float) (0.5*Konst*(distance_single));
			//float electrostatic=(float) ((332.0716*tempq1*tempq2)/distance);
			System.out.println("u nonbonded vand " +energy);
			System.out.println("distance"+distance_single);
			utotal_nonbonded=energy;
		 }
...
//      Set Alignment
	...
	private void bottomPanel()
    {
       ...
       //bottomPanel.setBorder(BorderFactory.createLineBorder(new Color(235,233,215),8));
        ...
        JLabel lbl = new JLabel("X Atom1", JLabel.LEFT);
	    slider_x = new JSlider(JSlider.HORIZONTAL,  -45, 45 , 0);
	    slider_x.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
		    	...
		    	move_single();
		   // 	System.out.println("DDIISSTTAANNCCEE " +distance_single);
		    	
		    	
		    	try {
			  	    utotal =0;
			  	  //  System.out.println("conforming "+utotal);
					try {
						nonbond_energy();
				...
				/*    ftotal=0;
					...
					System.out.println("Energg" + utotal_nonbonded);
								if ( distance_single > 0.0)	  
					{			
						
						Transform3D tr = new Transform3D();
				    	TransformGroup objtr=(TransformGroup)hm.get("atom1");
				    	//tr.setScale(new Vector3d(temp_Scalingx,Scaling,temp_Scaling));
				    	
			        	tr.setTranslation(new Vector3d(temp,magnet_y1_translate,magnet_z1_translate));
			    	...
			    	Energy.addGraphValue((float)(utotal_nonbonded*4));//-freeBody.getDisplacment()*500))
			    	Energy.drawGraph();
			    	Energy.setYScale(10);
			    	
			    	Force.setState(1);
			    	Force.setCurrentValue((float)disp/200,(float)ftotal_nonbond/10);        
			        Force.addGraphValue((float)ftotal_nonbond);//-freeBody.getDisplacment()*500));
			        Force.drawGraph();
			        Force.setYScale(10);
					}
		    }
	    ...
	    slider_x.setBackground(bk);
	   	...
	   	lbl = new JLabel("Y Atom1 ", JLabel.LEFT);
		    ...
		    slider_y.addChangeListener(new ChangeListener() {
				...
				//System.out.println("disp"+disp);
					...
					{	
				    	...
				    	tr.setTranslation(new Vector3d(magnet_x1_translate,temp,magnet_z1_translate));
			        	objtr.setTransform(tr);
			    	...
			    	Energy.setCurrentValue((float)disp/200,(float)((utotal)));      // Note that disp denotes the net displacement.  
			    	Energy.addGraphValue((float)(utotal_nonbonded*4));//-freeBody.getDisplacment()*500))
			    	Energy.drawGraph();
			    	Energy.setYScale(10);
			    	
			    	Force.setState(1);
			    	Force.setCurrentValue((float)disp/200,(float)ftotal/10);        
			        Force.addGraphValue((float)ftotal_nonbond);//-freeBody.getDisplacment()*500));
			        Force.drawGraph();
			        Force.setYScale(10);
	...
	slider_y.setBackground(bk);
	 ...
	 first.add(slider_y);
	 	...
	 	lbl = new JLabel("Z Atom1", JLabel.LEFT);
	 	
	    slider_z = new JSlider(JSlider.HORIZONTAL,  -30, 30 , 0);
	    slider_z.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
		    	...
		    	magnet_z1_translate=(float) temp;
		    	move_single();
		    //	System.out.println("DDIISSTTAANNCCEE " +distance_single);
				...
				/*    ftotal=0;
					...
					if ( distance_single > 0.0)	  
					{
			        	...
			        	tr.setTranslation(new Vector3d(magnet_x1_translate,magnet_y1_translate,temp));
...
/*	 javax.swing.JPanel temp;
	...
	*/ 
	...
	second=new JPanel(new java.awt.GridLayout(3,1,0,0));
	second.setBackground(bk);
	JLabel lbl1 = new JLabel("Sigma Atom1", JLabel.LEFT);
    slider_sg1 = new JSlider(JSlider.HORIZONTAL,  50,150 , 100);
    slider_sg1.addChangeListener(new ChangeListener() {
	    public void stateChanged(ChangeEvent e) {
	    	valChange = true;
	    	double temp=((double)((JSlider) e.getSource()).getValue())/10.0;
	    	Konst=(float)temp;
	    	
	    	//Transform3D tr = new Transform3D();
	    	//TransformGroup objtr=(TransformGroup)hm.get("atom1");
	    	
	    	//tr.setTranslation(new Vector3d(-magnet_x1_translate,-magnet_y_translate,-magnet_z1_translate));
	    	//objtr.setTransform(tr);
	    	//objtr.getTransform(tr);
	    	
	    	//tr.setScale(new Vector3d(temp*5,temp*5,temp*5));
	    	//objtr.setTransform(tr);
	    	//objtr.getTransform(tr);
    ...
    slider_sg1.setBackground(bk);
   	second.add(lbl1);
   	second.add(slider_sg1);
	   	
   	/*lbl1 = new JLabel("Sigma Atom2", JLabel.LEFT);
    slider_sg2 = new JSlider(JSlider.HORIZONTAL,  95,105 , 100);
    slider_sg2.addChangeListener(new ChangeListener() {
	    public void stateChanged(ChangeEvent e) {
	    	valChange = true;
	    	double temp=((double)((JSlider) e.getSource()).getValue())/1000.0;
	    	temprho2=(float) temp;
	    }
	    });
    slider_sg2.setBackground(bk);
   	second.add(lbl1);
   	second.add(slider_sg2);
   	
   	lbl1 = new JLabel("Episilon", JLabel.LEFT);
    slider_ep = new JSlider(JSlider.HORIZONTAL,  95,105 , 100);
    slider_ep.addChangeListener(new ChangeListener() {
	    public void stateChanged(ChangeEvent e) {
	    	valChange = true;
	    	double temp=((double)((JSlider) e.getSource()).getValue())/1000.0;
	    	tempepselon=(float) temp;
	    }
	    });
    slider_ep.setBackground(bk);
   	second.add(lbl1);
   	second.add(slider_ep);
   /*	
	 fifth=new JPanel(new java.awt.GridLayout(3,1,0,0));
	 fifth.setBackground(bk);
	 //temp=new JPanel(new java.awt.GridLayout(1,2,0,0));
	 //temp.setBackground(bk);
	 //lbl = new JLabel("Atom 2", JLabel.LEFT);
     //temp.add(lbl);
     //fifth.add(temp);
     
     //first=new JPanel(new java.awt.GridLayout(4,1,0,0));
     //first.setBackground(bk);
     lbl1 = new JLabel("X Atom2", JLabel.LEFT);
	    slider_x = new JSlider(JSlider.HORIZONTAL,  -45, 45 , 0);
	    slider_x.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
	...
	/*			if ( distance_single > 0.08)	  
	    ...
	    slider_x.setBackground(bk);
	   	...
	   	lbl = new JLabel("Y atom2 ", JLabel.LEFT);
	   	slider_y = new JSlider(JSlider.HORIZONTAL, -50*2, 50*2,0);
		    slider_y.addChangeListener(new ChangeListener() {
			    public void stateChanged(ChangeEvent e) {
			    	valChange = true;
	...
	slider_y.setBackground(bk);
	 ...
	 lbl1 = new JLabel("Z Atom2", JLabel.LEFT);
	    slider_z = new JSlider(JSlider.HORIZONTAL,  -45, 45 , 0);
	    slider_z.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
  ...
  */ 
        ...
        // *************************************for the movement
        	//Scaling=Scaling + 0.01;
          	Scaling=0.6-magnet_y_translate/2;
          	//double temp_Scaling=0.6+magnet_y_translate/2;
        	net_translation=-0.535;
            //	System.out.println("NET Translation is" + 0.54);
          	//double displacement= -0.535 -(lowest_atom_coordinates[1]* Scaling - 0.04);
          	
          	
          	//Scaling=0.6-magnet_x1_translate/2;
          	//double temp_Scalingx=0.6+magnet_x1_translate/2;
        	//net_translation=-0.535;
            //	System.out.println("NET Translation is" + 0.54);
          	//double displacementx= -0.535 -(lowest_atom_coordinates[1]* Scaling - 0.04);
         /* 	tr = new Transform3D();
            objtr=(TransformGroup)hm.get("support2");
            objtr.getTransform(tr);
            tr.setScale(new Vector3d(0.5,1,0.5));
