...
//        scene.addChild(bgleg);
	...
	static float ftotal_bend_keep=0;
...
/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
	...
	public static void energy_bend1 () throws NumberFormatException, IOException
	{
		int no=0;
		utotal_bend=0;
		int iter=0;
...
/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
	...
	public static void energy_bend2 () throws NumberFormatException, IOException
	{
		utotal_bend=0;
		for (int j=2;j<= iter_bend ;j++)
		{
			float rx=(temp[bend1[j]][0]-temp[bend2[j]][0]);
			float ry=(temp[bend1[j]][1]-temp[bend2[j]][1]);
			float rz=(temp[bend1[j]][2]-temp[bend2[j]][2]);
			float rij=(float) Math.sqrt((rx*rx)+(ry*ry)+(rz*rz));
			
			float rx1=(temp[bend3[j]][0]-temp[bend2[j]][0]);
			float ry1=(temp[bend3[j]][1]-temp[bend2[j]][1]);
			float rz1=(temp[bend3[j]][2]-temp[bend2[j]][2]);
	...
	/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
		...
		{
			ftotal_bend=0;
			//ftotal_bend=ftotal_bend_keep;
			float faltuf1=0,faltuf2=0,faltuf3=0;
			for (int j=2;j<= iter_bend ;j++)
			{
				float rxij=(temp[bend1[j]][0]-temp[bend2[j]][0]);
				float ryij=(temp[bend1[j]][1]-temp[bend2[j]][1]);
				float rzij=(temp[bend1[j]][2]-temp[bend2[j]][2]);
				float rij=(float) Math.sqrt((rxij*rxij)+(ryij*ryij)+(rzij*rzij));//adv : can call force b4 eng;
				
				float rxjk=(temp[bend3[j]][0]-temp[bend2[j]][0]);
				float ryjk=(temp[bend3[j]][1]-temp[bend2[j]][1]);
				float rzjk=(temp[bend3[j]][2]-temp[bend2[j]][2]);
    ...
    public static void nonbond_force ( int cas) throws NumberFormatException, IOException
	{
		...
		{
			counterrr++;
			ftotal_nonbond-=((fnet/counterrr)*magnet_y_translate);
		}
...
/*			if(result[0].equals("ATOM")) :CHANGE FORMAT WHEN NECCESSARY	*/
    ...
    public static void force_dihed()
	{
		...
		for (int j=2;j<= iter_dihed ;j++)
		{
			...
			float rx1=-(temp[dihed2[j]][0]-temp[dihed3[j]][0]);
			float ry1=-(temp[dihed2[j]][1]-temp[dihed3[j]][1]);
			float rz1=-(temp[dihed2[j]][2]-temp[dihed3[j]][2]);                       // check 2-3 or 3-2
			
			float rx2=-(temp[dihed3[j]][0]-temp[dihed4[j]][0]);
			float ry2=-(temp[dihed3[j]][1]-temp[dihed4[j]][1]);
			float rz2=-(temp[dihed3[j]][2]-temp[dihed4[j]][2]);                        // check 3-4 or 4-3
...
//      Set Alignment
    ...
    private void bottomPanel()
    {
       ...
       //bottomPanel.setBorder(BorderFactory.createLineBorder(new Color(235,233,215),8));
        ...
        JLabel lbl = new JLabel("X coordinate of Magnet ", JLabel.LEFT);
	   	...
	   	slider_y = new JSlider(JSlider.HORIZONTAL, -50*2, 50*2,0);
		    slider_y.addChangeListener(new ChangeListener() {
			    public void stateChanged(ChangeEvent e) {
			    	...
			    	//System.out.println("TEMP VARIABLE IS1 " +temp_y);
			    	double T_x=center_x[2]-center_x[1];
			    	double T_y=center_y[2] - center_y[1];
			    	double T1=(temp_y*Math.abs(T_x))/temp_r;
			    	double T2=(temp_y*Math.abs(T_y))/temp_r;
			    	temp_y=Math.sqrt(Math.pow(T1,2) + Math.pow(T2, 2));*/
			    	//System.out.println("TEMP VARIABLE IS2 " +temp_y);
			    	/*if(number_of_atoms >2 && number_of_atoms <4)
			    	{
			    		double temp_r1=(1+magnet_y_translate)*r0[2];
			    		System.out.println(temp_r1+ " "+r0[2]);
				    	double temp_y1=-1*k[2]*(temp_r1-r0[2]);
				    	double T_x1=center_x[2]-center_x[3];
				    	double T_y1=center_y[2] - center_y[3];
				    	double T11=(temp_y1*Math.abs(T_x1))/temp_r1;
				    	double T21=(temp_y1*Math.abs(T_y1))/temp_r1;
				    	temp_y1=Math.sqrt(Math.pow(T11,2) + Math.pow(T21, 2));
				    	temp_y+=temp_y1;
				    	//System.out.println("TEMP VARIABLE IS in bracket " +temp_y1);
			    	}
			    	
			   
			       // System.out.println("distance  " +temp_r  +"    "+"force"+temp_y);
			        temp_r=(1+magnet_y_translate)*r0[1];
			    	temp_y=(0.5*k[1]*(temp_r-r0[1])*(temp_r-r0[1]));
			    	if(number_of_atoms >2 && number_of_atoms <4)
			    	{
			    		temp_r+=(1+magnet_y_translate)*r0[2];
			    		temp_y+=(0.5*k[2]*(temp_r-r0[2])*(temp_r-r0[2]));
			    		
			    		//center_x
				    	double first_x=center_x[2]-center_x[1];
				    	double first_y=center_y[2]-center_y[1];
				    	double second_x=center_x[2]-center_x[3];
				    	double second_y=center_y[2]-center_y[3];
				    	
				    	double dot_product=first_x*second_x+first_y*second_y;
				    	double denominator=Math.sqrt(Math.pow(first_x,2)+Math.pow(first_y,2)) * Math.sqrt(Math.pow(second_x,2)+Math.pow(second_y,2));
				    	double init_answer=Math.acos(dot_product / denominator);
				    	
				    	
				    	first_x*=(1+magnet_y_translate);
				    	first_y*=(1-magnet_y_translate);
				    	second_x*=(1+magnet_y_translate);
				    	second_y*=(1-magnet_y_translate);
				    	
				    	dot_product=first_x*second_x+first_y*second_y;
				    	denominator=Math.sqrt(Math.pow(first_x,2)+Math.pow(first_y,2)) * Math.sqrt(Math.pow(second_x,2)+Math.pow(second_y,2));
				    	double final_answer=Math.acos(dot_product / denominator);
				    	//System.out.println("ANGLE IS "+init_answer+ " "+final_answer);
				    	
				    	double X=0.5 *  Math.pow(final_answer-init_answer,2);
				    	System.out.println("VALUE IS "+X);
				   	//System.out.println("VALUES AARE" + first_x +" "+first_y + " "+ second_x+ " "+second_y);
				    	temp_y+=X;
			    	}*/
			   /* 	try {
						read();
					} catch (NumberFormatException e1) {
						System.out.println("error in read - file cant open;");
			  ...
			  /*   try {
					...
					System.out.println("error in bonds - file cant open;");
	
				}*/
			  	   try {
			  	    utotal_nonbonded =0;
			  	    System.out.println("conforming "+utotal_bend);
					nonbond_energy2();
				} catch (NumberFormatException e1) {
				...
				if(prev>temp)
			    	...
			    	Energy.setCurrentValue((float)disp/100,(float)(10*(103+utotal_nonbonded)));      // Note that disp denotes the net displacement.  
			    	Energy.addGraphValue((float)(-10*(103+utotal_nonbonded)));//-freeBody.getDisplacment()*500))
			    	Energy.drawGraph();
			    	Energy.setYScale(10);
			    	
			    	Force.setState(1);
			       	//if(ftotal_nonbond>10000 ||ftotal_nonbond<-10000 )
			    	//	ftotal_nonbond=0;
			    	float tempr=3*ftotal_nonbond;
			        Force.addGraphValue(tempr);//-freeBody.getDisplacment()*500));
			        Force.drawGraph();
			        Force.setYScale(10);
			        ftotal_bend_keep=-ftotal_bend;
			        prev=temp;
			    }
		    });
	...
	slider_y.setBackground(bk);
	 ...
	 temp=new JPanel(new java.awt.GridLayout(1,2,0,0));
	 temp.setBackground(bk);
	 lbl = new JLabel("Type 0  c-c-c ", JLabel.LEFT);
     temp.add(lbl);
     k[1]=5;
     ...
     value_12.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
		    	double temp=((double)((JSlider) e.getSource()).getValue());
		    	System.out.println("k value "+bend_k[0]);
		    	epsilon[0]=(float) (temp*40);
		    }
	 });
     ...
     distance_12.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
		    	double temp=((double)((JSlider) e.getSource()).getValue());
		    	System.out.println("r value "+bend_theta[0]);
	 ...
	 temp=new JPanel(new java.awt.GridLayout(1,2,0,0));
	 temp.setBackground(bk);
	 lbl = new JLabel("Type 1 c-h-c", JLabel.LEFT);
     temp.add(lbl);
     ...
     value_23.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
		    	...
		    	System.out.println("1 k value "+bend_k[1]);
		    	bond_k[1]=temp2*40*4;
		    }
	 });
     ...
     distance_23.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent f) {
		    	...
		    	System.out.println("r value 2"+bend_theta[1]);
    ...
    /* value_34 = new JSlider(JSlider.HORIZONTAL,  1, 10 ,2);
     value_34.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
		    	double temp2=((double)((JSlider) e.getSource()).getValue());
		    	System.out.println("1 k value "+bend_k[2]);
		    	bond_k[1]=temp2*40*4;
		    }
	 });
     ...
     temp.add(value_34);
     distance_34 = new JSlider(JSlider.HORIZONTAL,  100, 200 ,154);
     distance_34.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent f) {
		    	double temp2=((double)((JSlider) f.getSource()).getValue());
		    	bond_r[1] =(float) (temp2/10000.0);//m*** to change value of r before exp.
		    	System.out.println("r value 2"+bend_theta[2]);
		    	System.out.println("my temp 2"+temp2);
		    }
	 });
